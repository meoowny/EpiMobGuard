...
def simulation_task(R0, I_H_para, I_R_para, H_R_para, I_input, region_list, simulation_days, simulation_city, cur_dir_name, policy_file_name, query_file_name):
    output_dir = f'./SimulationResult/MADDPG_result/{simulation_city}/{cur_dir_name}'
    ...
    data = {
        ...
        "simulationTask_time": query_file_name
    }
    ...
    if simulation_city == 'yuzhong':
        ...
        for time_index in range(simulation_days * 24):
            # 判断选择哪个时间段的action文件
            file_time_index = int(time_index / 4)
            action_file = gpd.GeoDataFrame.from_file(
                './grid_coefficient/' + str(simulation_city) + '/' + policy_file_name + '/' + str(
                    file_time_index * 4) + '-' + str(file_time_index * 4 + 3) + '_grid.shp')
            action_array = action_file['quota'].values
            action_array = action_array.reshape(-1, 1)
            S_0, I_0, H_0, R_0 = simulate_step(
                time_index, S_0, I_0, H_0, R_0, N_0, OD[time_index, :, :], [R0, I_H_para, I_R_para, H_R_para], grid_array, output_dir, action_array
            )
    else:
        for file_index in range(simulation_days * 6):
            ...
            action_file = gpd.GeoDataFrame.from_file(
                './grid_coefficient/' + str(simulation_city) + '/' + policy_file_name + '/' + str(
                    file_index * 4) + '-' + str(file_index * 4 + 3) + '_grid.shp')
            action_array = action_file['quota'].values
            action_array = action_array.reshape(-1, 1)
            for hour_index in range(4):
                S_0, I_0, H_0, R_0 = simulate_step(
                    file_index * 4 + hour_index, S_0, I_0, H_0, R_0, N_0, OD[:, :, hour_index],
                    [R0, I_H_para, I_R_para, H_R_para], grid_array, output_dir, action_array
                )
...
def simulate_step(time_index, S_0, I_0, H_0, R_0, N_0, OD, simulation_para, grid_array, output_dir, action_array):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--policy_file_name", type=str, required=True, help="policy_file_name")
    parser.add_argument("--query_file_name", type=str, required=True, help="query_file_name")
    ...
    result = simulation_task(
        ...
        args.I_input, args.region_list, args.simulation_days, args.simulation_city,args.cur_dir_name, args.policy_file_name, args.query_file_name
    )
    print(result)
