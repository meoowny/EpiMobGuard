...
// 初始化OpenLayer地图
export const initOLMap = (bindThis,id='Map') => {
  const that = bindThis
  // 获取地图容器
...
//测量交通压力并在地图上显示
export async function buildMapPressure(bindThis,index=1) {
  const that = bindThis
  //交通压力测度
  // try {
  const res = await that.getTrafficPressure(index)
  const trafficPressureResult = processPressureRoads(that, res)
  that.map.removeLayer(that.currentTrafficPressureLayer)
  if (that.showLayers.trafficPressure) {
...
function processPressureRoads(bindThis, data) {
  ...
  that.map.on('singleclick', (e) => {
    ...
    if (feature&&feature.name=="pressure") {
      that.$refs.mapInfoPlanPoint.showPopper = false
      // 设置弹窗位置
